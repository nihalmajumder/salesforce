...
trigger INT_Interaction on Interaction__c (after insert, after update) {
    ...
    // Add  code to turn off trigger
    List<Trigger_Settings__mdt> settings = [SELECT Turn_Off_Trigger__c FROM Trigger_Settings__mdt WHERE QualifiedApiName = 'Interaction'];
    if (settings == null || settings.isEmpty() || settings[0].Turn_Off_Trigger__c != true) {
